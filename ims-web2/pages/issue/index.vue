    ...
    <div>
      ...
      <label>
        <select v-model="selectedVersion">
          <option v-for="version in versions" v-bind:key="version.id" v-bind:value="version.id">
            {{ version.name }}
          </option>
        </select>
      </label>
      ...
      <label>
        <select v-model="selectedStatus">
          <option v-for="status in statuses"  v-bind:key="status.id" v-bind:value="status.id">
            {{ status.name }}
          </option>
        </select>
      </label>
...
<script>
  ...
  import Grid from '../../node_modules/@toast-ui/vue-grid/src/Grid.vue'
  export default {
    name: 'issue-page',
    components: {
      'grid': Grid
    ...
    methods: {
      ...
      searchCodeProjects: function () {
        this.$axios.get('/api/code/projects', {
          headers: { // 요청 헤더
      ...
      searchCodeProjectVersions: function () {
        this.$axios.get('/api/code/projects/' + this.selectedProject + '/versions', {
          headers: { // 요청 헤더
            'contentType': 'application/json',
            'Authorization': cookie.getCookie('token')
          }
